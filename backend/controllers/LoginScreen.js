...
// LoginScreen conectado al backend real
...
export default function LoginScreen({ navigation }) {
  ...
  const { setUser, isLoading, setIsLoading, error, setError, clearError } = useAuthStore();
  ...
  const onSubmit = async ({ email, password }) => {
    ...
    try {
      ...
      // Navegar al home autenticado
      navigation.replace('HomeAuth');

    } catch (err) {
      // err.data.pendiente = true → cuenta no aprobada aún
      if (err.data?.pendiente) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      await api.post(ENDPOINTS.FORGOT_PASSWORD, { email: forgotEmail.trim() });

      setForgotLoading(false);
      setForgotVisible(false);
      navigation.navigate('VerifyCode', { email: forgotEmail.trim() });
...
const styles = StyleSheet.create({
  ...
  rememberMeRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: SPACING.lg },
  rememberMeText: { fontSize: FONTS.sizes.md, color: COLORS.secondary, marginLeft: SPACING.sm, fontWeight: '600' },
  spacer: { flex: 1 },
